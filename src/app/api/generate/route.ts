...
export async function POST(request: Request) {
    try {
        ...
        if (type === 'meaning') {
            ...
            systemInstruction = 'あなたは優秀な英語講師です。英単語・熟語の難易度に合わせた自然な英語の例文を作成してください。文脈が分かりやすく、実用的な文章にしてください。対象の単語・熟語の形（時制、単複、品詞など）は、最も一般的で自然な使われ方に適宜変形して使用して構いません。出力は英語の例文のみとし、挨拶、日本語訳、思考プロセス、前置きなどは一切含めないでください。例文は長すぎず、1文のみで、10〜18単語程度の簡潔で明瞭な文章にしてください。毎回異なる視点や状況を想定し、バリエーション豊かな例文を出力してください。';
            const situations = [
                // TOEIC最頻出シチュエーション 5選
