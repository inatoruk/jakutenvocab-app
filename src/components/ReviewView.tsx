...
export default function ReviewView({ active, settings, vocabVersion = 0 }: { active: boolean; settings: AppSettings; vocabVersion?: number }) {
    ...
    async function handleRegisterParaphrase() {
        ...
        try {
            ...
            // 親となる出題カードをパラフレーズカテゴリに移す
            if (currentCard.category !== "Paraphrase") {
                const { error: updateCategoryError } = await supabase
                    .from("vocab")
                    .update({ category: "Paraphrase" })
            ...
            setAllCards(prev => {
                const updatedPrev = prev.map(c => 
                    c.id === currentCard.id ? { ...c, category: "Paraphrase" } : c
                );
                return [...updatedPrev, newVocab];
            });
    ...
    async function handleMastered() {
        ...
        setAllCards((prev) =>
            prev.map((c) => (c.id === currentCard.id ? { ...c, status: newStatus, review_due_at: newDueAt } : c))
        );
                            ...
                            {!showAnswer ? (
                                ...
                                <>
                                    <div className="flex-1 flex flex-col justify-center space-y-2">
                                        ...
                                        {paraphraseResult !== null && (
                                            <div className={`text-center rounded-lg px-4 py-2 text-sm font-semibold flex flex-col gap-1 ${
                                                paraphraseResult === "correct"
                                                    ? "bg-green-50 dark:bg-green-800/40 text-green-700 dark:text-green-200 border border-green-200 dark:border-green-700/50"
                                                    : paraphraseResult === "synonym"
                                                        ? "bg-blue-50 dark:bg-blue-800/40 text-blue-700 dark:text-blue-200 border border-blue-200 dark:border-blue-700/50"
                                                        : "bg-red-50 dark:bg-red-800/40 text-red-700 dark:text-red-200 border border-red-200 dark:border-red-700/50"
                                            }`}>
                                                <span className="flex items-center justify-center gap-1.5 py-0.5">
                                                    ...
                                                    {paraphraseResult === "synonym" && (
                                                        <>
                                                            <Circle size={15} className="stroke-[3]" />
                                                            <span>Nice synonym! (登録外の正解)</span>
                                                        </>
                                                    )}
                                        ...
                                        {siblings.length > 0 && (
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex flex-wrap items-center justify-center gap-2">
                                                    {siblings.map((s) => (
                                                        <div key={s.id} className="rounded-lg bg-violet-50 dark:bg-violet-800/40 border border-violet-100 dark:border-violet-700/50 px-3 py-1.5">
                                        ...
                                        {aiHint && !aiChecking && (
                                            <div className="flex items-start gap-2 rounded-lg bg-amber-50 dark:bg-amber-800/40 border border-amber-100 dark:border-amber-700/50 px-3 py-2">
                                    ...
                                    <div className="flex justify-center gap-3 pt-3 shrink-0">
                                        ...
                                        <button
                                            onClick={goNext}
                                            className="flex-1 max-w-[200px] inline-flex items-center justify-center gap-1 rounded-lg border border-violet-300 dark:border-violet-700/60 bg-violet-50 dark:bg-violet-800/40 px-4 py-2.5 text-sm font-medium text-violet-700 dark:text-violet-200 hover:bg-violet-100 dark:hover:bg-violet-900/60 active:bg-violet-200 dark:active:bg-violet-900/70 transition-colors duration-200"
                                        >
                                            次へ
                        ...
                        {!showAnswer ? (
                            ...
                            <>
                                ...
                                <div className="flex justify-center gap-3 pt-3 shrink-0">
                                    ...
                                    <button
                                        onClick={handleKeep}
                                        className="flex-1 max-w-[140px] rounded-lg border border-orange-300 dark:border-orange-700/60 bg-orange-50 dark:bg-orange-800/40 px-4 py-3 text-sm font-medium text-orange-700 dark:text-orange-200 hover:bg-orange-100 dark:hover:bg-orange-900/60 active:bg-orange-200 dark:active:bg-orange-900/70"
                                    >
                                        まだ（Keep）
                                    </button>
                                    {(reviewMode === "unlearned" || reviewMode === "writing") ? (
                                        <button
                                            ...
                                            className="flex-1 max-w-[140px] rounded-lg border border-green-300 dark:border-green-700/60 bg-green-50 dark:bg-green-800/40 px-4 py-3 text-sm font-medium text-green-700 dark:text-green-200 hover:bg-green-100 dark:hover:bg-green-900/60 active:bg-green-200 dark:active:bg-green-900/70"
                                        >
                                            覚えた（Mastered）
                                        </button>
                                    ) : (
                                        <button
                                            onClick={goNext}
                                            className="flex-1 max-w-[140px] inline-flex items-center justify-center gap-1 rounded-lg border border-blue-300 dark:border-blue-700/60 bg-blue-50 dark:bg-blue-800/40 px-4 py-3 text-sm font-medium text-blue-700 dark:text-blue-200 hover:bg-blue-100 dark:hover:bg-blue-900/60 active:bg-blue-200 dark:active:bg-blue-900/70"
                                        >
                                            次へ
                                            <ChevronRight size={16} />
                                        </button>
